...
export async function POST(req: Request) {
  try {
    ...
    if (!actief) {
      await supabase.from('organisatie_addons').delete()
        .eq('organisatie_id', organisatie_id).eq('addon_id', addon_id)
      return NextResponse.json({ removed: true })
    }
    const { data, error } = await supabase.from('organisatie_addons')
      .upsert({ organisatie_id, addon_id, aantal: aantal ?? 1, actief: true, gestart_op: new Date().toISOString().split('T')[0] },
